...
fn process_commands(display: &mut Display, input: Vec<u64>) {
    ...
    while i < input.len() {
        if input[i] == 1 {
            ...
            if input[i + 1] > display.max_height - 1 || input[i + 2] > display.max_width - 1 {
                panic!("Выход за допустимые границы дисплея.")
            }
            current_point = (input[i + 1], input[i + 2]);
            i += 3;
        } else if input[i] == 2 {
            if i + 1 == input.len() {
                panic!("Неверно задана последовательность действий.");
            }
            let c = match input[i + 1] {
                1 => 1,
                2 => 2,
                3 => 3,
            ...
            display
                .matrix
                .set_colour(current_point.0, current_point.1, c);
            i += 2;
        } else {
            panic!("Неверно задана последовательность действий.");
        }
