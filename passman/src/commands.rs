...
static HELP_TEXT: &str =
    "Available commands: exit, help, add, show, show all, change, delete, delete all, delete file.";
...
pub fn add(mpass: &str) -> Result<()> {
    ...
    if find(&resource, mpass).is_ok() {
        println!(
            "Record for resource {} already exists. Use change command.",
            resource
        );
        return Ok(());
    }
...
pub fn delete_all() -> Result<()> {
    ...
    let line = reader
        ...
        .next()
        .unwrap()
        .map_err(|_| Errors::CorruptedData)?;
    file = OpenOptions::new()
        .write(true)
        .truncate(true)
        .open(FILE_NAME)
...
fn type_name() -> String {
    ...
    loop {
        ...
        if !re.is_match(&input) {
            println!(
                "Resource name must consist of numbers, Latin letters and symbols: '.', '-' and '@'."
            );
            input.clear();
        } else {
...
fn add_record(resource: &str, password: &str, mpass: &str) -> Result<()> {
    ...
    let encrypted_data = encrypt(
        format!("{} {}", resource, password).as_bytes(),
        &generate_crypto_key(mpass),
        &init_vec,
    )
    .map_err(|_| Errors::Unspecified)?;
    insert(
        &HEXUPPER.encode(&init_vec),
        &HEXUPPER.encode(&encrypted_data),
    )?;
    Ok(())
}
...
fn find(resource: &str, mpass: &str) -> Result<(usize, String)> {
    ...
    for (index, line) in reader.lines().enumerate().skip(1) {
        let line = line.map_err(|_| Errors::CorruptedData)?;
        let decrypted_data = decrypt_data(line, &generate_crypto_key(mpass))?;
        let (f_resource, f_password) = decrypted_data
            .split_once(" ")
            .ok_or(Errors::CorruptedData)?;
...
fn encrypt(data: &[u8], key: &[u8], init_vec: &[u8]) -> Result<Vec<u8>> {
    Ok(
        cbc::Encryptor::<aes::Aes256>::new(key.into(), init_vec.into())
            .encrypt_padded_vec_mut::<Pkcs7>(data),
    )
}

fn decrypt(data: &[u8], key: &[u8], init_vec: &[u8]) -> Result<Vec<u8>> {
    cbc::Decryptor::<aes::Aes256>::new(key.into(), init_vec.into())
